...
ScriptGenerator.prototype.generateAspNetCoreDeploymentScript = function (_) {
  ...
  var nuget_35_renamed = "nuget3_5.exe" // TODO rename the nuget.exe 3.5, default nuget.exe is 4.0 and above
  var nugetRestoreArgs;
  ...
  if (this.solutionPath && !(this.projectPath.endsWith(".csproj"))) {
    ...
    if (fs.lstatSync(this.absoluteProjectPath).isDirectory()) {
        ...
        // TODO move this logic to kudu script
    }
    ...
    if (this.scriptType == ScriptType.batch) {
      ...
      nugetRestoreArgs = "%DEPLOYMENT_SOURCE%\\" + this.solutionPath;
    } else {
      msbuildArguments = '"$DEPLOYMENT_SOURCE\\' + this.solutionPath + '" /nologo /verbosity:m /p:AutoParameterizationWebConfigConnectionStrings=false`;Configuration=Release`;UseSharedCompilation=false` $SCM_BUILD_ARGS';
      dotnetpublishArguments = '"$DEPLOYMENT_SOURCE\\' + this.projectPath + '" /nologo /verbosity:m /t:GatherAllFilesToPublish /p:AutoParameterizationWebConfigConnectionStrings=false`;Configuration=Release`;UseSharedCompilation=false`;PublishOutputPathNoTrailingSlash="$DEPLOYMENT_TEMP" $SCM_BUILD_ARGS';
      nugetRestoreArgs = "$DEPLOYMENT_SOURCE\\" + this.solutionPath;
    ...
    var options = {
      ...
      restore: nuget_35_renamed + ' restore "' + nugetRestoreArgs + '" -packagesavemode nuspec'
    };

    this.generateAspNetCoreScript('aspnet.core.msbuild.template', options, _);
    // only .sln + xproj can be catergorized to OLD BUILD SYSTEM
  } else {
    nugetRestoreArgs = ' restore "' + ((this.solutionPath) ? this.solutionPath:this.projectPath) + '"';
    var buildAndPublish;
    // no more dotnet restore
    if(this.projectPath.endsWith(".csproj")) {
      // use nuget4.0 and msbuild \t publish
      nugetRestore = 'nuget.exe' + nugetRestoreArgs;
      if (this.scriptType == ScriptType.batch) {
        buildAndPublish = '"%MSBUILD_PATH%" "' + fixPathSeperatorToWindows(this.projectPath) + '" /t:Publish /p:PublishDir="%DEPLOYMENT_TEMP%";Configuration=Release %SCM_BUILD_ARGS%';
      }else{
        buildAndPublish = '"$MSBUILD_PATH" "' + fixPathSeperatorToWindows(this.projectPath) + '" /t:Publish /p:PublishDir="$DEPLOYMENT_TEMP";Configuration=Release $SCM_BUILD_ARGS';
      }
    }else{
      // use nuget3.5 and dotnet publish
      nugetRestore = nuget_35_renamed + nugetRestoreArgs;
      if(this.scriptType == ScriptType.batch){
        buildAndPublish = 'dotnet publish "' + fixPathSeperatorToWindows(this.projectPath) + '" --output "%DEPLOYMENT_TEMP%" --configuration Release';
      }else{
        buildAndPublish = 'dotnet publish "' + fixPathSeperatorToWindows(this.projectPath) + '" --output "$DEPLOYMENT_TEMP" --configuration Release';
      }
    }
    this.generateAspNetCoreScript('aspnet.core.template', {
...
ScriptGenerator.prototype.generateAspNetCoreScript = function (templateFileName, options, _) {
  ...
  var templateContent = getTemplatesContent([
    ...
    'deploy.' + lowerCaseScriptType + '.' + templateFileName,
    'deploy.' + lowerCaseScriptType + '.postfix.template'
]).replace(/{BuildAndPublish}/g, options.buildAndPublish)
